...
from logic import Expr, PropSymbolExpr, parseExpr, pl_true, pycoSAT, to_cnf
import util
...
def sentence1() -> Expr:
    ...
    return conjoin(first, sec, third)
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    return conjoin(*[(~l[0] | ~l[1]) for l in itertools.combinations(literals, 2)]) 
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    # Pacman takes exactly one action at timestep t. 
    pacphysics_sentences.append(exactlyOne([PropSymbolExpr(dir, time=t) for dir in DIRECTIONS]))
    ...
    if successorAxioms is not None and t:
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    pacPos = PropSymbolExpr(pacman_str, x1, y1, time=1)
    return findModel(conjoin(KB) & pacPos), findModel(conjoin(KB) & ~pacPos)
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for x, y in non_wall_coords:
            ...
            fod = PropSymbolExpr(food_str, x, y, time=t)
            persistant_food = PropSymbolExpr(food_str, x, y, time=t + 1) 
            KB.append(pacmanSuccessorAxiomSingle(x, y, t+1, walls))

            KB.append((pac & fod) >> ~persistant_food)
            KB.append((~pac & fod) >> persistant_food)
            

# helper function 1
def h1(t, agent, KB, all_coords, non_outer_wall_coords, walls_grid):
...
def h2(t, KB, x, y, possible_locations):
    pacloc = PropSymbolExpr(pacman_str, x, y, time=t)
    cnfKB = conjoin(KB)
    if findModel(cnfKB & pacloc):
        possible_locations.append((x, y))
    elif entails(cnfKB, pacloc): 
        KB.append(pacloc)
    else:
        KB.append(~pacloc)

def h3(KB, non_outer_wall_coords, known_map):
    for x, y in non_outer_wall_coords:
        potential_wall = PropSymbolExpr(wall_str, x, y) 
        cnfKB = conjoin(KB)
        if entails(cnfKB, potential_wall):
            known_map[x][y] = 1
            KB.append(potential_wall)
        elif entails(cnfKB, ~potential_wall):
            KB.append(~potential_wall)
            known_map[x][y] = 0 
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        # Add pacphysics, action and percept information to KB
        h1(t, agent, KB, all_coords, non_outer_wall_coords, walls_grid)

        # Find possible pacman locations with updated KB
        possible_locations = []
        for x, y in non_outer_wall_coords:
            h2(t, KB, x, y, possible_locations)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        h1(t, agent, KB, all_coords, non_outer_wall_coords, known_map)
        
        h3(KB, non_outer_wall_coords, known_map)
