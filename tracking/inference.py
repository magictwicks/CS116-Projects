...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict = {}
    possibleLocations = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    
    variableDomainsDict[PAC] = possibleLocations
    variableDomainsDict[GHOST0] = possibleLocations
    variableDomainsDict[GHOST1] = possibleLocations
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            factors, squished_factors = joinFactorsByVariable(factors, var)
            if len(squished_factors.unconditionedVariables()) == 1:
                pass
            else: 
                factors.append(eliminate(squished_factors, var))
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        range = [0]
        for key in key_list:
            range.append(range[-1] + self[key]) # creating a list that represents the ranges of probability for the given distribution
        
        sample = random.random() * range[-1]
        choice = 0
        while sample > range[choice]:
            choice += 1
        
        return key_list[choice - 1]
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        # got from curtis.... have no clue why this is needed... 
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for old_pos in self.beliefs.keys():
            ...
            for new_pos in newPosDist.keys():
                belief_dist[new_pos] += newPosDist[new_pos] * self.beliefs[old_pos]
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        if check0 == 0:
            ...
            weights.normalize()
            newParticleDistribution = [weights.sample() for particle in range(self.numParticles)]
            self.particles = newParticleDistribution
